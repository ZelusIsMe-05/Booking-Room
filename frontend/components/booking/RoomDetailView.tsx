...
interface RoomDetailViewProps {
  room: any;
  /** Right-column content — booking checkout (tenant) or an edit panel (host). */
  sidebar: ReactNode;
  /** Optional "back" link rendered above the gallery. */
  backLink?: { href: string; label: string };
  /** Optional full-width content rendered below the two-column section (e.g. reviews). */
...
export default function RoomDetailView({ room, sidebar, backLink, bottomSlot }: RoomDetailViewProps) {
  ...
  return (
    <>
      ...
      {backLink && (
        <div className="mb-4">
          <Link href={backLink.href} className="text-sm font-bold text-booking-primary transition hover:text-booking-primaryDark">
            ← {backLink.label}
          </Link>
        </div>
      )}
      ...
      <section className="mt-8 grid gap-8 lg:grid-cols-[1.38fr_0.62fr] items-start">
        ...
        {/* Right Column (Sticky Sidebar — provided by caller) */}
        {sidebar}
      </section>

      {/* Optional full-width content below (e.g. reviews) */}
      {bottomSlot && <section className="mt-5">{bottomSlot}</section>}
